...
namespace TvdbLib
{
  ...
  public class TvdbDownloader
  {
    ...
    public TvdbDownloader(String _apiKey)
    {
      ...
      #region Proxy
      if (MediaManager.Master.Settings.XML.Config.confProxy.UseProxy)
      {
          WebProxy wProxy = new WebProxy(MediaManager.Master.Settings.XML.Config.confProxy.ProxyAdress, Convert.ToInt32(MediaManager.Master.Settings.XML.Config.confProxy.ProxyPort));
          wProxy.Credentials = new NetworkCredential(MediaManager.Master.Settings.XML.Config.confProxy.ProxyUser, MediaManager.Master.Settings.XML.Config.confProxy.ProxyPassword, MediaManager.Master.Settings.XML.Config.confProxy.ProxyDomain);
